  ...
  useImperativeHandle(ref, () => ({
    move: (value) => {
      setPosition((prevPosition) => ({
        x: Math.min(Math.max(prevPosition.x + value, 0), 450), // Adjust max width as needed
        y: prevPosition.y,
      }));
    },
    turn: (value) => {
    ...
    glideToRandomPosition: () => {
      ...
      const animate = (time) => {
        ...
        const newX =
          initialPosition.x + (randomX - initialPosition.x) * easedProgress;
        const newY =
          initialPosition.y + (randomY - initialPosition.y) * easedProgress;
